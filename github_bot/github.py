...
def has_signature(comment: GithubComment, slug: str) -> bool:
    return (
        comment["user"]["login"] in ["github-actions[bot]", "martin-more"]
        and SIGNATURE_TEMPLATE.format(slug=slug) in comment["body"]
    )


def get_comments_link(
    pr_number: int, repo_name: Optional[str] = None, org_name: Optional[str] = None
) -> str:
    url = f"/repos/{org_name}/{repo_name}/issues/{pr_number}/comments"
...
def get_labels_link(
    ...
    url = f"/repos/{org_name}/{repo_name}/issues/{pr_number}/labels"
...
def get_comments(
    ...
    comments_link = get_comments_link(pr_number, repo_name=repo_name, org_name=org_name)
...
def get_labels(
    pr_number, repo_name: Optional[str] = None, org_name: Optional[str] = None
) -> List[GithubLabel]:
    labels_link = get_labels_link(pr_number, repo_name=repo_name, org_name=org_name)
...
def add_or_edit_comment(
    ...
    current_comments = get_comments(pr_number, repo_name=repo_name, org_name=org_name)
    ...
    url = (
        ...
        if previous_comment
        else get_comments_link(pr_number, repo_name=repo_name, org_name=org_name)
    )
...
def update_labels(
    ...
    labels_url = get_labels_link(pr_number, repo_name=repo_name, org_name=org_name)

    current_labels_url_by_name = {
        label["name"]: label["url"]
        for label in get_labels(pr_number, repo_name=repo_name, org_name=org_name)
    }
