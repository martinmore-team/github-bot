...
class Mutation:
    ...
    def add_ok_to_preview_comment(self, input: AddOkToPreviewCommentInput) -> str:
        ...
        links = [
            f"https://example.com/docs/pr/{input.pr_number}/{path}" for path in paths
        ]
    ...
    def add_release_file_comment(self, input: AddReleaseFileCommentInput) -> str:
        ...
        if input.status == ReleaseFileStatus.MISSING:
            comment = comment.format(repo_name=input.repo_name)
